...
int main() {
  ...
  // preset time between meas, IRL it would be based on input data
  double sensor_range = 50;  // Sensor range [m]

  // GPS measurement uncertainty [x [m], y [m], theta [rad]]
  double sigma_pos [3] = {0.3, 0.3, 0.01};
  // Unintuitively, we are also using the same sigma
  // for adding uncertainty in the prediction step
  
  // Landmark measurement uncertainty [x [m], y [m]]
  double sigma_landmark [2] = {0.3, 0.3};
  // control data is noiseless (assumption/simplification)
