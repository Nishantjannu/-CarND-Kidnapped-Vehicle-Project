...
struct LandmarkObs {
  ...
  double landmark_x; // Global x position of nearest landmark [m]
  double landmark_y; // Global y position of nearest landmark [m]
};
...
inline double multiv_prob(double sig_x, double sig_y, double x_obs, double y_obs,
  ...
  return (1 / (2 * M_PI * sig_x * sig_y))*
            exp(-((pow(x_obs - mu_x, 2) / (2 * pow(sig_x, 2))) + (pow(y_obs - mu_y, 2) / (2 * pow(sig_y, 2)))));
}  
